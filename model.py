...
class Transformer(nn.Module):
    """
    """
    def __init__(self, encoder: Encoder, decoder: Decoder, src_embed: WordEmbeddings, tgt_embed: WordEmbeddings, src_pos: PositionalEncoding, tgt_pos: PositionalEncoding, projection_layer: ProjectionLayer) -> None:
        super().__init__()
...
def build_transformer(src_vocab_size: int, tgt_vocab_size: int, src_seq_len: int, tgt_seq_len: int, d_model: int = 512, N: int = 6, h: int = 8, dropout: float = 0.1, d_ff: int = 2048):
    src_embed = WordEmbeddings(d_model, src_vocab_size)
    tgt_embed = WordEmbeddings(d_model, tgt_vocab_size)
    src_pos = PositionalEncoding(d_model, src_seq_len, dropout)
    tgt_pos = PositionalEncoding(d_model, tgt_seq_len, dropout)

    encoder_blocks = []
    for _ in range(N):
        encoder_self_attention_block = MultiHeadAttentionBlock(d_mdoel, h, dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
        encoder_blocks.append(EncoderBlock(encoder_self_attention_block, feed_forward_block, dropout))

    decoder_blocks = []
    for _ in range(N):
        decoder_self_attention_block = MultiHeadAttentionBlock(d_mdoel, h, dropout)
        decoder_cross_attention_block = MultiHeadAttentionBlock(d_mdoel, h, dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
    ...
    transformer = Transformer(encoder, decoder, src_embed, tgt_embed, src_pos, tgt_pos, src_pos, tgt_pos, projection_layer)
